      ...
      <main class="bg-white p-16 pb-12">
        ...
        >
          ...
          <v-table class="mt-4">
            ...
            <tbody>
              <tr v-for="(record, i) in wordList.records" :key="i">
                ...
                <td>未実装</td>
                <td>
                  {{ record.length }} {{ $t("records.tableData.length") }}
                </td>
                <td>{{ record.date }}</td>
                <td>
                  <v-btn size="small" @click="deleteRecord(index)">
                    {{ $t("records.tableData.actions.delete") }}
                  </v-btn>
                </td>
              </tr>
              <p v-if="wordList.records.length === 0" class="text-sm mt-4">
                まだ記録がありません。
              </p>
            </tbody>
          </v-table>
